...
<?php
    ...
    if(isset($_POST['nom'])) { // If the form is filled
        ...
        if($row['count']) { // If the user is already existing in the database
            ...
            $sql=$bd->prepare('INSERT INTO Utilisateur (idRole,nom,prenom,mail,mot_de_passe,idPromotion) VALUES (?,?,?,?,?,?)');
            $nom=htmlspecialchars($_POST['nom']);
            $prenom=htmlspecialchars($_POST['prenom']);
            $idPromotion=htmlspecialchars($_POST['promotion']);
...
<body id="body">
    ...
    <div id="main"> <!-- The main page -->
        ...
        <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>"> <!-- Form to add a user -->
            ...
            <br>
            <select id="promotion" name="promotion" class="form-control" required> <!-- List of the roles -->
                <?php
                    $bd=openBD();
